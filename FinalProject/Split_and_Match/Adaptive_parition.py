...
#original_img='irnia.jpg'
original_img='google.png'
#original_img='teddy.jpg'
#original_img = 'elefant.jpg'
example_img='style.jpg'
#example_img='V.jpeg'
...
def parition_concept(Patch_position,patchNO,Each_Patch_Size,std_dev,LEAF_NODES):
	...
	possible_patches= extract_patches_2d(dum_V, Each_Patch_Size) ## In V:
	Ti=Patch.shape
...
for ri in range(4):
	...
	print('Ri, PATCH.shape :',ri,PATCH.shape)
	print("**************************************************\n")
	std_dev=np.std(PATCH)
...
def find_labels(pa,dum_U,dum_V,img_yuv_V):
	...
	for key in K_centers :
		...
		cv2.rectangle(temp_v,(vertices[0][0],vertices[0][1]),(vertices[1][0],vertices[1][1]),(255,255,0),1)
		k_labels_vertices.append(vertices)

	print(k_labels_vertices,len(k_labels_vertices))

	cv2.rectangle(temp_u,(int(pa[0][1]),int(pa[0][0])),(int(pa[1][1]),int(pa[1][0])),(255,255,0),1)
	#cv2.imshow('VisualizationV',temp_v)
	#cv2.imshow('VisualizationU',temp_u)
	cv2.imwrite('visual1.jpg',temp_v)
	#cv2.waitKey(0)
	lb[str(pa)]=k_labels_vertices
...
########### Dumping into Files ###############
lf=open('leaf_nodes.txt','w')
d={}
for k,v in nodes_dict.iteritems():
	print(k,v)
	print("\n\n")
	lf.write(str(k)+":"+str(v)+"\n")
	d[k]=v
...
for nodes in nodes_dict :
	for vertices in nodes_dict[nodes] :
		...
		print(vertices, "\n")
		cv2.rectangle(o_img,(vertices[0][1],vertices[0][0]),(vertices[1][1],vertices[1][0]),(255,255,0),1)

for vertices in all_regions:
    cv2.rectangle(o_img,(vertices[0][1],vertices[0][0]),(vertices[1][1],vertices[1][0]),(255,255,0),1)

cv2.imshow('Visualization',o_img)
cv2.imwrite('splitPatches_teddy.jpg',o_img)
cv2.waitKey(0)
...
json.dump(lb, open("labels.txt",'w'))
